...
class Sql
{
   ...
   /**
     ...
     *
     * @assert ('date')->text() [==] 'LAST_DAY(`date`)'
     * @assert ('date', 'm')->text() [==] 'LAST_DAY(`date`) AS `m`'
     *
     * @param string      $column_name
     * @param string|null $alias       ชื่อรองที่ต้องการ ถ้าไม่ระบุไม่มีชื่อรอง
     *
     * @return \static
     */
    public static function LAST_DAY($column_name, $alias = null)
    {
        return self::create('LAST_DAY('.self::fieldName($column_name).')'.($alias ? " AS `$alias`" : ''));
    ...
    /**
     * ฟังก์ชั่นสร้างคำสั่ง IF
     *
     * @assert ('create_date', 'U.create_date')->text() [==] "IF(`create_date`, U.`create_date`)"
     * @assert ('create_date', 'U.create_date', 'test')->text() [==] "IF(`create_date`, U.`create_date`) AS `test`"
     *
     * @param string      $column_name1
     * @param string      $column_name2
     * @param string      $column_name3
     * @param string      $column_name4
     * @param string|null $alias        ถ้าระบุจะมีการเติม alias ให้กับคำสั่ง
     *
     * @return \self
     */
    ...
    public static function IF($column_name1, $column_name2,$column_name3,$column_name4, $alias = null)
    {
        return self::create('IF('.self::fieldName($column_name1).'= '.self::fieldName($column_name2).','.self::fieldName($column_name3).','.self::fieldName($column_name4).')'.($alias ? ' AS `'.$alias.'`' : ''));
    }
    public static function IFOVER($column_name1, $column_name2,$column_name3,$column_name4, $alias = null)
    {
        return self::create('IF('.self::fieldName($column_name1).'>'.self::fieldName($column_name2).','.self::fieldName($column_name3).','.self::fieldName($column_name4).')'.($alias ? ' AS `'.$alias.'`' : ''));
    }
    /**
     * ฟังก์ชั่นสร้างคำสั่ง IFMINUTES
     *
     * floor(IF(TIMESTAMPDIFF(SECOND, R.`create_date`, (SELECT `create_date` FROM `repair`.`rp_repair_status` WHERE `id` = T.`max_id`))/ 3600 >=1 
     * (TIMESTAMPDIFF(SECOND, R.`create_date`, (SELECT `create_date` FROM `repair`.`rp_repair_status` WHERE `id` = T.`max_id`)) 
     *  -TIMESTAMPDIFF(HOUR, R.`create_date`, (SELECT `create_date` FROM `repair`.`rp_repair_status` WHERE `id` = T.`max_id`))*3600)/60 ,0)) as  min
     *
     * @param string      $column_name1
     * @param string      $column_name2
     * @param string      $column_name3
    ...
    public static function IFMINUTES($column_name1, $column_name2,$column_name3,$column_name4) //, $alias = null
    {
        return self::create('floor(IF('.self::fieldName($column_name1).'/ 3600 >=1 , ('.self::fieldName($column_name2).'-'.self::fieldName($column_name3).'*3600)/60 ,'.self::fieldName($column_name4).'))'); //.(' AS `minutes`')
    }
    public static function IFHOUR($column_name1, $column_name2,$column_name3,$column_name4) //, $alias = null
    {
        return self::create('floor(IF('.self::fieldName($column_name1).'/ 3600 >=1 , ('.self::fieldName($column_name2).'-'.self::fieldName($column_name3).'*86400)/3600 ,'.self::fieldName($column_name4).'))');//.(' AS `hours`')
    }
    public static function IFSECOND($column_name1, $column_name2) //, $alias = null
    {
        return self::create('floor('.self::fieldName($column_name1).'-'.self::fieldName($column_name2).'*60)'); //.(' AS `seconds`')
    }
    ...
    {
        return self::create('SUM('.self::fieldName($column_name).')'.($alias ? " AS `$alias`" : ''));
    //.($distinct ? 'DISTINCT ' : '')
    }
    ...
    public static function UNIX_TIMESTAMP($column_name1, $alias = null)
    {
        return self::create('UNIX_TIMESTAMP('.self::fieldName($column_name1).')'.($alias ? " AS `$alias`" : ''));
    }
    ...
    public static function fieldName($column_name)
    {
        if ($column_name instanceof self) {
            ...
            if (preg_match('/^(([A-Z]{1,1}[0-9]{0,1})\.)?`?([a-zA-Z0-9_]+)`?(\s(ASC|DESC|asc|desc))?$/', $column_name, $match)) {
